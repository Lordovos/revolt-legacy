mob/unit
	...
	var/list/abilities
	// The previous list of possible moves generated by flood_fill().
	var/list/moves
...
mob/unit/proc/Unselect(client/c)
	if (c.selected_unit.moves)
		for (var/turf/t in c.selected_unit.moves)
...
mob/unit/proc/RenderMoves()
	src.moves = ::flood_fill(src.loc, src.move)

	for (var/turf/t in src.moves)
		if (src.moves[t] > src.move)
			continue
